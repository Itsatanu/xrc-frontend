...
export default function UpdateDeposit() {
    ...
    const URL = `${API_URL}/api/v1/admin/update-deposit`
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (data.id === '') {
            setError("Deposit ID can't be empty")
            return
        }
            ...
            <div className='admin-panel-update-deposit-withdrawal'>
                {
                    data?.id ?
                        (
                            <div>
                                <h4 className='update-deposit-withdrawal-details'>Deposit ID: {data.id}</h4>
                                ...
                                <form className='update-deposit-withdrawal-form' onSubmit={handleSubmit} >
                                    ...
                                    <button>{stateLoading ? <LazyLoading /> : 'Update Deposit'}</button>
                        ...
                        (
                            <div style={{ alignContent: 'center' }}>
                                <h2>Please come with proper Deposit ID</h2>
                            </div>
                        )
                }
            </div>
